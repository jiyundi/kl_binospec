...
class EmissionProcessor:
    ...
    def process(self, line, this_line_waves, 
        ...
        specs = [self.specA.copy(), self.specC.copy(), self.specB.copy()]
        centers, d2g_params = {}, {}
        
        for i, spec in enumerate(specs):
            centers[f'Set{i}'] = {}
            for lineidx in range(len(this_line_waves)):
                centers[f'Set{i}'][f'line{lineidx+1}'] = None
            ...
            if line == "O2":
                ...
                centers[f'Set{i}']['line1'] = {
                    'y': round(y0, 2), 'x': round(x1, 2)
                    }
                centers[f'Set{i}']['line2'] = {
                    'y': round(y0, 2), 'x': round(x2, 2)
                    }
        ...
        for i in range(len(specs)):
            ...
            nlines = 2 if line == 'O2' else 1
            for j_line in range(nlines): # support max = doublet line
                how_cut[f'Set{i}'][f'line{j_line+1}'] = {
                    'y0': round(centers[f'Set{i}'][f'line{j_line+1}']['y'] - UP, 2),
                    'x0': round(centers[f'Set{i}'][f'line{j_line+1}']['x'] - LF, 2),
                    }
    ...
    def crop(self, centers, line, spec_width, spec_height):
        ...
        for i, spec2d in enumerate(self.specs):
            ...
            try:
                ...
                cx = int(round(np.mean([
                    centers[f'Set{i}'][f'line{lineidx+1}']['x']
