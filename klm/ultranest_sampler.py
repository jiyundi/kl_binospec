...
class UltranestSampler(KLInference):
        ...
        for i, key in enumerate(self.config.params.names):
            ...
            #    params[i] = 10**self.truncnormprior.ppf(cube[i])
                ...
                try:
                    ...
                    params[i] = self.config.params.prior[key].ppf(cube[i])
    ...
    def run(self, output_dir='./ultranest_output/', test_run=False, 
            if_continue_last_run='subfolder', run_num=1, **kwargs):
        ...
        sampler_kwargs = {}
        if self.config.truevalues is not None:
            sampler_kwargs["truevalues"] = self.config.truevalues
        ...
        sampler = ultranest.ReactiveNestedSampler(
                            ...
                            # truevalues=self.config.truevalues,
                            log_dir=output_dir, 
                            resume=if_continue_last_run,
                            run_num=run_num, **sampler_kwargs)
        ...
        if test_run is False:
            sampler.run(min_num_live_points=400, max_ncalls=1e8) # , max_iters=28000

        elif test_run is True:
            print('Testing likelihood evaluations...')
            sampler.run(min_num_live_points=400, max_iters=50, show_status=True)
