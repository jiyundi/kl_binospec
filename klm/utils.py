...
def calculate_spec_snr(spec, sky_var):
    '''
    Calculates the SNR of a spec
    '''
    
    return np.sum(spec) / (np.sum(spec) + np.sum(sky_var))**0.5
...
def calculate_image_snr(image, var):
    ...
    return np.sum(image) / np.sum(var)**0.5
...
def predcit_shear_from_map(RA, Dec, shear_def, z_source, z_lens=0.225,method='nfw', path=None, return_error=False):
    '''
    ...
    assert shear_def in ['g', 'gamma'], 'Shear definition must be either reduced shear g or gamma'
    ...
    kappa_map = galsim.fits.read(f'{path}/hlsp_clash_model_a2261_zitrin-{method}_v2_kappa.fits')
#     kappa_err_map = galsim.fits.read(f'{path}/hlsp_clash_model_a2261_zitrin-{method}_v2_kappa-1sigmaerr.fits')

    coord = galsim.CelestialCoord(RA*galsim.degrees, Dec*galsim.degrees)
    objpos = gamma1_map.wcs.toImage(coord)

    # kappa_predict, kappa_predict_err = 0., 0.
...
#         gamma2_predict_err = gamma2_err_map[objpos.round()] * beta/beta_A2261_CLASH
        gamma1_predict = gamma1_map[int(objpos.x), int(objpos.y)] * beta/beta_A2261_CLASH
        gamma1_predict_err = gamma1_err_map[int(objpos.x), int(objpos.y)] * beta/beta_A2261_CLASH
        gamma2_predict = gamma2_map[int(objpos.x), int(objpos.y)] * beta/beta_A2261_CLASH
        gamma2_predict_err = gamma2_err_map[int(objpos.x), int(objpos.y)] * beta/beta_A2261_CLASH

        if shear_def == 'g':
            kappa_predict =  kappa_map[int(objpos.x), int(objpos.y)] * beta/beta_A2261_CLASH
#             kappa_predict_err = kappa_err_map[int(objpos.x), int(objpos.y)] * beta/beta_A2261_CLASH
            print(f'kappa {method}: ', kappa_predict)

        gamma1_predict = gamma1_predict/ (1 - kappa_predict)
        gamma2_predict = gamma2_predict/ (1 - kappa_predict)

        if return_error is True:
            return gamma1_predict, gamma2_predict, gamma1_predict_err, gamma2_predict_err#, kappa_predict, kappa_predict_err
...
def get_gammat_var_in_cutout(RA, Dec, R, z_source, z_lens=0.225, method='nfw', path=None):
    ...
    ap_coord = SkyCoord(260.6032*u.degree, 32.1084*u.degree)
    phi = get_PA(ap_coord).to(u.radian).value

    # Modify to account for source position
    beta = get_lensing_depth(z_source, z_lens)
    beta_A2261_CLASH = 1.
    kappa_map = kappa_map.array * beta/beta_A2261_CLASH

    # Compute reduced shear
    g1 = gamma1_map.array/(1 - kappa_map)
    g2 = gamma2_map.array/(1 - kappa_map)

    gammat = -g1 * np.cos(2*phi) + g2 * np.sin(2*phi)
...
#     gammat_cutout = gammat[pix_y-R_pix:pix_y+R_pix, pix_x-R_pix:pix_x+R_pix]
    ...
    c1 = CirclePixelRegion(center=PixCoord(x=pix_x, y=pix_y), radius=R_pix)
    mask = c1.to_mask().to_image(gammat.shape).astype(bool)
    gammat_cutout = np.ma.masked_where(~mask, gammat)
    gammat_cutout = gammat_cutout[gammat_cutout.mask==False]
