...
if __name__ == '__main__':
    ...
    for slit_num in slit_nums:
        ...
        for spec_idx in range(len(data_info['spec'])):
            ...
            p1_1_3_arr  = data_info['spec'][spec_idx]['mask']
            p1_1_4_dict = data_info['spec'][spec_idx]['meta']
            ...
            flux_spec     = np.where(p1_1_3_arr, (p1_1_1_arr + p1_1_2_arr), np.nan)
            flux_image    = np.where(  p2_3_arr, (  p2_1_arr +   p2_3_arr), np.nan)
            mask_nega_var1= np.where(p1_1_3_arr, (p1_1_3_arr & ((p1_1_1_arr + p1_1_2_arr) <= 0)).astype(int), np.nan)
            mask_nega_var2= np.where(  p2_3_arr, (  p2_3_arr & ((  p2_1_arr +   p2_2_arr) <= 0)).astype(int), np.nan)
            im0 = ax[0,0].imshow(flux_spec,     aspect='auto', cmap='viridis')
            im1 = ax[0,1].imshow(mask_nega_var1,aspect='auto', cmap='viridis')
            im2 = ax[1,0].imshow(flux_spec,     aspect='auto', cmap='viridis')
            im3 = ax[1,1].imshow(mask_nega_var2,aspect='auto', cmap='viridis')
            print(f'Slit {slit_num}: Bad px in Set {spec_idx}:', 
                  p1_1_1_arr[p1_1_3_arr & ((p1_1_1_arr + p1_1_2_arr) <= 0)], 
                  f'(var = {np.nanmean(np.where(p1_1_3_arr, p1_1_2_arr, np.nan)):.2f})')
            if np.any(p2_3_arr & ((  p2_1_arr +   p2_2_arr) <= 0)):
                print(f'Slit {slit_num}: Bad px in image:', 
                        p2_3_arr[  p2_3_arr & ((  p2_1_arr +   p2_2_arr) <= 0)], 
                      f'(var = {np.nanmean(np.where(p2_3_arr, p2_2_arr, np.nan)):.2f})')
            plt.colorbar(im0, ax=ax[0,0])
            plt.colorbar(im1, ax=ax[0,1])
            plt.colorbar(im2, ax=ax[1,0])
            plt.colorbar(im3, ax=ax[1,1])
            ax[0,0].set_title(f'Slit {slit_num}: Set {spec_idx}')
            ax[0,1].set_title('Pixels of data + var <= 0')
            plt.show()
